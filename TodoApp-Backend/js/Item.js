...
$(document).ready(function () {
    ...
    $.ajax({
        type: "GET",
        url: "http://localhost/Server_Side_CW1/TodoApp-Backend/api/item/getitems?listId=4",
        success: function (response) {
            ...
            if (response == "Enter complete user information to save" | response == "Email already found") {
                ...
                $.each(response, function (i) {
                    $('#list').append('<li class="list-group-item">' + response[i].ItemName + '<button type="button" ' +
